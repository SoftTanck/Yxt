...
public class RoundProgressView extends View {
    ...
    public RoundProgressView(Context context, AttributeSet attrs, int defStyle) {
        ...
        textIsDisplayable = mTypedArray.getBoolean(R.styleable.RoundProgressView_textIsDisplayable, true);
        style = mTypedArray.getInt(R.styleable.RoundProgressView_style, 0);
        mTypedArray.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        RectF oval1 = new RectF(centre - radius, centre - radius, centre
                + radius, centre + radius);  //用于定义的圆弧的形状和大小的界限
        paint.setColor(roundColor); //设置圆环的颜色
        canvas.drawArc(oval1, 135, 270, false, paint);  //根据进度画圆弧
        ...
        switch (style) {
            case STROKE:
                ...
                canvas.drawArc(oval, 135, 270 * progress / max, false, paint);  //根据进度画圆弧
                break;
            case FILL:
                paint.setStyle(Paint.Style.FILL_AND_STROKE);
                // if(progress !=0)
                canvas.drawArc(oval, 135, 270 * progress / max, true, paint);  //根据进度画圆弧
                break;
        }
